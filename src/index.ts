...
async function mainMenu() {
  ...
  let vaultData: Record<string, string> = session.data;
  let masterKey: Buffer = session.key;
  let salt: Buffer = session.salt;
        ...
        if (selectedService !== 'back') {
         const pass = vaultData[selectedService] || '';
          ...
          try {
            ...
            setTimeout(() => {
              try { clipboardy.default.writeSync(''); } catch (e) { }
            }, 10000);
          } catch (err) {
